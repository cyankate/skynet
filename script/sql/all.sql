CREATE TABLE `mail` (
  ...
  `mail_type` tinyint NOT NULL COMMENT '邮件类型:1=系统邮件,2=玩家邮件,3=公会邮件,4=系统奖励邮件,5=全局邮件',
  `title` varchar(50) NOT NULL COMMENT '邮件标题',
  `content` varchar(1000) NOT NULL COMMENT '邮件内容',
  `items` text COMMENT '附件JSON格式',
  `items_status` tinyint NOT NULL DEFAULT '0' COMMENT '附件是否已领取:0=未领取,1=已领取',
  `status` tinyint NOT NULL DEFAULT '0' COMMENT '邮件状态:0=未读,1=已读',
