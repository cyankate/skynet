...
function M.start_match(player_id, options)
    local instanceS = skynet.localname(".instance")
    ...
    local prepare_ok, profiles_or_err = match_mode_adapter_mgr.prepare_team(matched_players, match_options)
    if not prepare_ok then
        skynet.send(instanceS, "lua", "clear_match_flow", matched_players)
        for _, pid in ipairs(matched_players) do
            notify_match_tip({ pid }, match_options.type_name or "", tostring(profiles_or_err or "队伍校验失败，匹配已取消"))
        end
        return false, profiles_or_err or "队伍校验失败，匹配已取消"
    end
...
function M.confirm_match(player_id, accept)
    local instanceS = skynet.localname(".instance")
    ...
    if event == "confirm_progress" then
        local pending_match = result.pending_match or {}
    ...
    if event == "confirm_all_accepted" then
...
function M.cancel_match(player_id)
    local instanceS = skynet.localname(".instance")
